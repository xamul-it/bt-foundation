...
LOG = logging.getLogger("long_window_close_sim")
REPO_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_MINUTE_DATA_DIR = REPO_ROOT / "config-common/data/m/alpaca/sip"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Long simulation: buy on entry, sell at window close.")
    ...
    p.add_argument("--output-dir", default="results/long_window_close_sim")
...
def simulate_long(df: pd.DataFrame, horizon: int, entry_mode: str) -> pd.DataFrame:
    ...
    ret_long = pd.Series(np.nan, index=df.index, dtype=float)
    ret_long.loc[entered] = (close_end.loc[entered] / entry.loc[entered]) - 1.0
...
def main() -> None:
    ...
    meta.to_csv(out_dir / "long_sim_meta.csv", index=False)
    ...
    for h in horizons:
        for mode in ["entry_open_first_bar", "entry_close_calc_bar", "entry_low_calc_bar"]:
            rows.append(simulate_long(sdf, h, mode))

    rr = pd.concat(rows, ignore_index=True)
    rr.to_csv(out_dir / "long_row_level.csv", index=False)
        ...
        summ_rows.append(
            {
                ...
                "mean_ret_long_given_enter": float(ent["ret_long"].mean()) if n_ent > 0 else np.nan,
                "median_ret_long_given_enter": float(ent["ret_long"].median()) if n_ent > 0 else np.nan,
                "winrate_long_given_enter": float(ent["win_long"].mean()) if n_ent > 0 else np.nan,
            }
        )

    summary = pd.DataFrame(summ_rows).sort_values(["entry_mode", "horizon_m"])
    summary.to_csv(out_dir / "long_summary.csv", index=False)

    # Plots
    n_sel = int(len(sdf))

    fig, ax = plt.subplots(figsize=(9, 5))
    sns.lineplot(data=summary, x="horizon_m", y="mean_ret_long_given_enter", hue="entry_mode", marker="o", ax=ax)
    ax.axhline(0.0, color="#444", linewidth=1)
    ax.set_title(f"Long mean return (given enter) | n_selected={n_sel}")
    ax.set_xlabel("Horizon (min)")
    ax.set_ylabel("Mean return")
    fig.tight_layout()
    fig.savefig(out_dir / "long_mean_return_by_horizon.png", dpi=150)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(9, 5))
    sns.lineplot(data=summary, x="horizon_m", y="winrate_long_given_enter", hue="entry_mode", marker="o", ax=ax)
    ax.set_ylim(0, 1)
    ax.set_title(f"Long winrate (given enter) | n_selected={n_sel}")
    ax.set_xlabel("Horizon (min)")
    ax.set_ylabel("Winrate")
    fig.tight_layout()
    fig.savefig(out_dir / "long_winrate_by_horizon.png", dpi=150)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "long_p_enter_by_horizon.png", dpi=150)
    plt.close(fig)

    LOG.info("Long simulation complete. Output: %s", out_dir)
